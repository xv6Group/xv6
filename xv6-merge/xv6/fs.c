...
bmap(struct inode *ip, uint bn)
{
  ...
  struct buf *bp;
 // uint i;	
  int a1, a2, a3, d1, d2, k1, k2, k3;	
  //uint* addr_ptr;
  if(bn < NDIRECT){
    if((addr = ip->addrs[bn]) == 0)
    {
      ip->addrs[bn] = addr = balloc(ip->dev);
      //cprintf("bmap error!~0");
    }
    return addr;
  }
  bn -= NDIRECT;
...
//cprintf("reading block %dover", addr);
 a = (uint*)bp->data;

 if((a1 = a[k3]) == 0)
 {
    a1 = a[k3] = balloc(ip->dev);
  // cprintf("error1");
 }
 brelse(bp);
//cprintf("reading block %dover", a1);
 bp = bread(ip->dev, a1);
 a = (uint*)bp->data;

 if((a2 = a[k2]) == 0)
 {
  a2 = a[k2] = balloc(ip->dev);
//cprintf("error1");
 }

 brelse(bp);
//cprintf("reading block %dover", a2);
 bp = bread(ip->dev, a2);
 a = (uint*)bp->data;

 if((a3 = a[k1]) == 0)
 {
  a3 = a[k1] = balloc(ip->dev);
//cprintf("error1");
 }
//cprintf("return addr:%d", a3);
brelse(bp);
 return a3;
}
